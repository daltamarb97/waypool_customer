...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__driverFindride__ = __webpack_require__(903);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_15__services_d_metrics_service__ = __webpack_require__(361);
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
...
var DriverFindridePage = /** @class */ (function () {
    ...
    DriverFindridePage.prototype.loadMap = function () {
        ...
        this.geolocation.getCurrentPosition({ enableHighAccuracy: true }).then(function (position) {
            ...
            _this.originCoords = { lat: position.coords.latitude, lng: position.coords.longitude };
            console.log(_this.originCoords);
            var mapOptions = {
                center: latLng,
                zoom: 17,
    ...
    DriverFindridePage.prototype.selectSearchResultMyPos = function (item) {
        ...
        this.geocoder.geocode({ 'placeId': item.place_id }, function (results, status) {
            if (status === 'OK' && results[0]) {
                ...
                _this.myLatLng = results[0].geometry.location;
                _this.usingGeolocation = false;
                _this.autocompleteMyPos.input = [item.description];
                _this.autocompleteMyDest.input = '';
                _this.originName = item.description;
                _this.originCoords = { lat: results[0].geometry.location.lat(), lng: results[0].geometry.location.lng() };
            }
        });
    };
    ...
    DriverFindridePage.prototype.selectSearchResultMyDest = function (item) {
        ...
        this.geocoder.geocode({ 'placeId': item.place_id }, function (results, status) {
            if (status === 'OK' && results[0]) {
                ...
                _this.destinationName = item.description;
                _this.destinationCoords = { lat: results[0].geometry.location.lat(), lng: results[0].geometry.location.lng() };
        ...
        this.geocoder.geocode({ 'location': latLng }, function (results, status) {
            if (status === 'OK') {
                if (results[0]) {
                    ...
                    _this.originName = results[0].formatted_address;
                }
                else {
    ...
    DriverFindridePage.prototype.listride = function () {
        ...
        if (this.currentUser.emailVerified == false) {
            ...
            this.afDB.database.ref('/driversTest/' + this.user).once('value').then(function (snapUser) {
                ...
                if (snapUser.val().documents.carne === false || snapUser.val().documents.id === false) {
                    ...
                    try {
                        ...
                        if (_this.autocompleteMyDest.input == '' || _this.autocompleteMyPos.input == '') {
                            ...
                            _this.sendCoordsService.pushcoordinatesDrivers(_this.user, _this.desFirebase, _this.orFirebase);
                            _this.originInfo = { coords: _this.originCoords, name: _this.originName };
                            _this.destinationInfo = { coords: _this.destinationCoords, name: _this.destinationName };
                            _this.afDB.database.ref('/reservesTest/' + _this.user).push({
                                driver: _this.userInfo,
                                origin: _this.originInfo,
                                destination: _this.destinationInfo,
                            }).then(function (snap1) {
                                var key1 = snap1.key;
